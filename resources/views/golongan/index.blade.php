...
<head>
    <meta charset="utf-8" />
    <title>Manajemen Data Golongan</title>
                ...
                <ul id="accordion-menu">
                    ...
                    <li class="dropdown">
                        ...
                        <ul class="submenu">
                            ...
                            <li><a href="{{ route('admin.golongan.index') }}" class="active">Kelas/Golongan</a></li>
                        </ul>
                    </li>
                </ul>
            </div>
        ...
        <div class="pd-ltr-20 xs-pd-20-10">
            <div class="min-height-200px">
                ...
                <div class="pd-20 card-box mb-30">
                    <div class="clearfix">
                        <div class="pull-left">
                            <h4 class="text-blue h4">Input Data Golongan</h4>
                            <p class="mb-30">Masukkan nama golongan baru ke dalam sistem</p>
                        </div>
                    </div>
                    <form id="golonganForm" method="POST">
                        @csrf
                        <div class="form-group">
                            <label>Nama Golongan <span class="text-danger">*</span></label>
                            <input class="form-control" name="nama_Gol" id="nama_Gol" type="text" placeholder="Contoh: A" maxlength="100" required />
                        </div>
                        <div class="form-group">
                ...
                {{-- TABEL DAFTAR GOLONGAN --}}
                <div class="card-box mb-30">
                    <div class="pd-20">
                        <h4 class="text-blue h4">Daftar Golongan</h4>
                        <p class="mb-30">Data golongan yang telah tersimpan dalam sistem</p>
                    </div>
                    <div class="pb-20">
                        <div class="table-responsive">
                            <table class="table table-striped">
                                <thead>
                                    <tr>
                                        <th scope="col" style="width: 10%;">ID Golongan</th>
                                    ...
                                    <tr id="row_{{ $item->id_Gol }}">
                                        ...
                                        <td>
                                            <div class="dropdown">
                                                ...
                                                <div class="dropdown-menu dropdown-menu-right dropdown-menu-icon-list">
                                                    <a class="dropdown-item btn-edit" href="javascript:void(0)" data-id="{{ $item->id_Gol }}"><i class="dw dw-edit2"></i> Edit</a>
                                                    <a class="dropdown-item btn-delete" href="javascript:void(0)" data-id="{{ $item->id_Gol }}"><i class="dw dw-delete-3"></i> Hapus</a>
                                                </div>
                                            </div>
                                        </td>
                                    </tr>
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form id="editForm">
                        @csrf
                        @method('PUT')
                        <input type="hidden" id="edit_id_Gol" name="id_Gol">
                        <div class="form-group">
                            <label>Nama Golongan <span class="text-danger">*</span></label>
                            <input class="form-control" name="nama_Gol" id="edit_nama_Gol" type="text" required />
                        </div>
                    </form>
                </div>
    ...
    <script>
        $(document).ready(function() {
            ...
            $('#golonganForm').on('submit', function(e) {
                e.preventDefault();
                $.ajax({
                    url: "{{ route('admin.golongan.store') }}",
                    type: 'POST',
                    data: $(this).serialize(),
                    success: function(response) {
                        if (response.success) {
                            ...
                            var newRow = `
                                <tr id="row_${response.data.id_Gol}">
                                    <td>${response.data.id_Gol}</td>
                                    <td>${response.data.nama_Gol}</td>
                                    <td>
                                        <div class="dropdown">
                                            <a class="btn btn-link font-24 p-0 line-height-1 no-arrow dropdown-toggle" href="#" role="button" data-toggle="dropdown"><i class="dw dw-more"></i></a>
                                            <div class="dropdown-menu dropdown-menu-right dropdown-menu-icon-list">
                                                <a class="dropdown-item btn-edit" href="javascript:void(0)" data-id="${response.data.id_Gol}"><i class="dw dw-edit2"></i> Edit</a>
                                                <a class="dropdown-item btn-delete" href="javascript:void(0)" data-id="${response.data.id_Gol}"><i class="dw dw-delete-3"></i> Hapus</a>
                                            </div>
                                        </div>
                                    </td>
                                </tr>`;
            ...
            $(document).on('click', '.btn-edit', function() {
                ...
                $.get("{{ route('admin.golongan.index') }}/" + id + "/edit", function(data) {
                    $('#editModal').modal('show');
                    $('#edit_id_Gol').val(data.id_Gol);
                    $('#edit_nama_Gol').val(data.nama_Gol);
                }).fail(function(xhr) {
                    Swal.fire({
                        icon: 'error',
                        title: 'Gagal!',
                        text: 'Tidak dapat mengambil data golongan.'
                    });
                });
            });
            ...
            $('#updateBtn').on('click', function() {
                var id = $('#edit_id_Gol').val();
                $.ajax({
                    // Perbaikan URL update menggunakan route admin
                    url: "{{ route('admin.golongan.index') }}/" + id,
                    type: 'PUT',
                    ...
                    success: function(response) {
                        if (response.success) {
                            ...
                            // Update row dengan data baru - lebih efisien
                            $('#row_' + id + ' td:nth-child(2)').text(response.data.nama_Gol);
                        }
                    },
                    error: function(xhr) {
